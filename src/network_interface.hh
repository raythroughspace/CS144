...
private:
  ...
  std::map<Address, EthernetAddress> mappings_ {};
  std::map<Address, uint32_t> mappings_TMO_ {};

  std::map<Address, uint32_t> sent_arp_msgs_{}; // sent arp messages with timeout

  std::map<Address, std::vector<InternetDatagram>> dgram_queue_{}; // stores datagrams that are awaiting arp reply to be sent
};
